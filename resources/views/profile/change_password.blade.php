...
@include('error')
  ...
  <h1><i class="fab fa-expeditedssl text-info "></i> Change Your Password</h1>
@endsection
@section('content')

<form class="col-md-6" action="{!!route('profile.change')!!}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
<input  type="hidden" name="id" value="{{\Auth::user()->id}}">
      ...
      <div class="form-group"> 
        ...
        <input type="password" name="new_password" placeholder="Enter Your New Password"  class="form-control" value="{{ old('quantity') }}">
      </div>
      <div class="form-group">
        
        <label><i class="fas fa-lock text-info m-1"></i><i class="fas fa-lock text-info"></i> Confirm Password</label>
        <input type="password" name="confirm_password" placeholder="Confirm Your New Password"  class="form-control" value="{{ old('quantity') }}">
      </div>
            <div class="form-group">
